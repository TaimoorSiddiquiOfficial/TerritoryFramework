...
| `ScenarioID` | — | `FName` | `—` | Stable story encounter ID, for example Blacksmith_BeforeHandover. Required for owner reinforcements. Use the same ID in the assault condition that unlocks handover. A completed encounter cannot spawn again; a cancelled attempt can be retried. | — |
