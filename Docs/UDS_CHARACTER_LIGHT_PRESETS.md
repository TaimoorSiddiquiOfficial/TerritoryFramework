...
Open the new profile's **Rig Class**, `BP_TerritoryUDSCharacterLightRig`. Under **01 UDS Full Light Presets**, choose a **Data Table** and **Row Name** for **Day Preset** and **Night Preset**. The starting selections are `Day_Studio` and `Night_Moonlight` in the project `DT_NarrativeLightRigPresets` table beside the profile. Duplicate the Blueprint and profile for a different scene's defaults. **Set UDS Presets** can change both row handles on an active local rig.

Use a complete rig preset table. The adapter applies its global settings and every character-light setting, including light type, color, intensity, shadows, target and wrap light. Its inherited parent creates the original `BP_LightRigElement` children. It removes background elements so a local conversation cannot change tagged lights elsewhere in the level. It does not apply a global post-process preset. TDA has a focused two-connection fix in the local pack's `BP_LightRigElement`; see [the element preset repair](LIGHT_RIG_ELEMENT_PRESET_REPAIR.md). The [complete pack audit](CHARACTER_LIGHT_RIG_FULL_SYSTEM_AUDIT.md) also records fixes to the panel's PP table refresh and custom skeleton dropdown, plus the remaining feature gaps. Narrative Pro and UDS assets remain unchanged by this integration.
