...
All six builds and 337 tests per engine pass, including Native ASC behavior,
save serialization and routed level teardown. Listen-host/two-client and
dedicated PIE/two-client runs each pass nine tag-count/session checks.
The UE 5.8 iterative cook/stage and 60-second packaged Development game
server-mode smoke pass. This is not a compiled dedicated-server target.
The original avatar's separate tag, cached leader context, camera/input cleanup
and immediate join/start timing remain open.
See [party setup and limits](PARTY_DIALOGUE_REPLY_RULES.md).

The preceding follow-up validates Native party transfers and keeps the actor's
member/relevance lists synchronized with its Tales component. A refused departure,
invalid identity, duplicate player or story callback redirect cannot produce a
successful destination join. Six builds and 334 tests per engine pass, including
Native save serialization and load-order regressions. Listen-server and dedicated
PIE each pass ten live transfer/playback checks. Speaker tags, cached leader
context, camera/input cleanup and immediate join/start timing remain open.
UE 5.8 iterative cook/stage and the 60-second packaged Development game
server-mode smoke pass. This is not a compiled dedicated-server target.
See [party setup and limits](PARTY_DIALOGUE_REPLY_RULES.md).

The preceding follow-up detaches a departing member's personal dialogue reference
...
checks. Per-member camera/input, voice/shot and separate avatar-tag cleanup,
active leader context migration remain open. Transfer membership checks are now
covered by the newest follow-up above.
...
UE 5.8 cook/stage and a 60-second packaged server-mode smoke also pass.
The chosen departure policy is **continue for remaining members**; its per-player
cleanup and active leader migration remain open.

Character Light Rig research is recorded in the
[compatibility and integration report](CHARACTER_LIGHT_RIG_COMPATIBILITY.md).
The runtime pack needs explicit Native visual/camera binding: the supplied
MetaHuman row expects `face`, while the live Narrative visual has `FaceMesh`.
...
The existing server observer now follows one exact Native Tales component.
Native membership events install one observer for the party. A rejected
replacement invokes Native's group exit once, clearing member aliases and both
clients. Each local presentation observes its current party's start, finish and
speaker events, and unbinds on leave or switch. Stale leave/finish events cannot
replace current membership. Native retains sessions, membership, message routing
and persistence; no vendor source, Blueprint signature, saved field or RPC changed.

The 327-test suites, all six builds and the live host/two-client regression pass.
This verifies session state and presentation flags; Hashir's greeting has no
authored camera shot or voice track. Native's leader-only reply setting is
UI-only, and departing members can retain Native dialogue aliases. Those remain
separate integration tasks before multiplayer story decisions are certified.
